        ...
        <Navbar className={`${styles.navbarAdmin} d-flex flex-wrap h-auto `} >
            ...
            <div className="down h-auto d-flex col-md-12">
                <div className="col-md-10 align-items-center offset-1 d-flex justify-content-center">
                    ...
                    <NavDropdown title={<span className={styles.navbuttonAdmin} > Categorias </span>} >
                    {categories.length>0?categories.map(c => <NavDropdown.Item onClick={() => {
                        selectCategory(c.name);
                        history.push('/products');
                    }} style={{ color: 'white' }}> {c.name} </NavDropdown.Item>):<h6 className={styles.navbarLinkAdmin}>No hay categorías creadas</h6>}
                    </NavDropdown>
